...
| Capacité système maximale (disques + stockage objet) ^1^ | 500 Gio | 2 Pio ^2^ | 2 Tio | 10 Tio | 368 Tio ^2^ | 368 Tio par licence ^2^ 
...
Les bandes passantes indiquées dans le tableau ci-dessous correspondent aux limites AWS documentées pour chaque type d'instance. Ces limites ne correspondent pas entièrement à ce que Cloud Volumes ONTAP peut fournir. Pour connaître les performances attendues, consultez https://www.netapp.com/pdf.html?item=/media/9088-tr4383.pdf["Rapport technique NetApp 4383 : caractérisation des performances de Cloud Volumes ONTAP dans Amazon Web Services avec des charges de travail applicatives"^].
